...
// but attempting to use [proto.Unmarshal] with a message created from it will
// panic. Instead, use [Type.Unmarshal].
//
// # Support Status
//
// This package does not implement large parts of the Go Protobuf reflection
// API, because it is specialized for parsing and reading only. More of the
// API may be implemented as time goes on. In general, the following operations
// are not supported or not implemented:
//
//   - Unmarshaling with [proto.Unmarshal], and unmarshaling onto an existing
//     message.
//   - Clearing and mutating messages, including [protoreflect.Message].NewField
//     and message merging and cloning.
//   - Unknown fields: unknown fields are discarded and cannot be set.
...
// to do so:
//
//   - Extensions (they will need to be precompiled with [Compile], rather than
//     resolved at parse time).
//   - Maps (currently they are rendered as repeated fields).
//   - Oneofs (currently each oneof member is rendered as an optional field).
//   - Groups (groups will parse as unknown fields and be discarded).
package fastpb
...
// # Hacking
//
// This package has benchmarks that compare against both protobuf-go's generated
// code and dynamicpb. To get a CPU profile, run
//
//   go test -bench . -test.run=Bench -o /tmp/fastpb.bin -test.cpuprofile /tmp/fastpb.prof
//   go tool pprof -http localhost:8000 /tmp/fastpb.bin /tmp/fastpb.prof
//
// This will run the benchmarks and open the captured profile in your browser.
//
...
// entity.
//
// The main loop of the interpreter is [parser1.message], which is not actually
// a loop but rather a strongly connected component of blocks joined by gotos.
